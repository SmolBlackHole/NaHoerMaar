...
defineProps<{ contributor: ListenerProfile | null; compact?: boolean }>();
...
<template>
	<span
		...
		:title="contributor ? `Added by ${contributor.name}` : 'No contributor recorded'"
	>
		<UAvatar
			v-if="contributor"
		...
		<span v-if="contributor" class="truncate"
			><span :class="compact ? 'sr-only' : ''">Added by </span
